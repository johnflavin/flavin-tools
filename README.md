...
## claude-copy.py
Recovers the clean source markdown from text copied out of the Claude Code TUI (in
Ghostty), ready to paste into Obsidian. Solves the "garbled mess" you get otherwise:
lines prefixed with two spaces, paragraphs hard-wrapped into a pile of newlines, and
all the inline formatting (bold, `code`, headings, fences) flattened away by the
renderer.

It reads the clipboard's plain text and produces markdown two ways, best-first:
...
1. **Transcript recovery (high fidelity).** Uses the copied text as a search needle
   ...
   backticks, links, code fences, headings, lists — exactly as Claude emitted it.

   The matcher is deliberately markup-agnostic. Instead of enumerating every way
   markdown markers can differ between the rendered TUI text and the raw source, it
   reduces **both** sides to their *alphanumeric skeleton* (letters and digits only) —
   so every marker, all whitespace, the soft-wrapping and the indent vanish uniformly,
   with no per-marker code. It fuzzy-aligns the skeletons (stdlib `difflib`) to find
   where the selection starts and ends in the raw source, expands those two boundaries
   outward to swallow a leading `##`/`**`/opening-fence and trailing punctuation/fence,
   and returns the raw slice between them verbatim. The middle never has to line up
   character for character, so a stray marker there can't break the match — and things
   like snake\_case in inline code, fence language labels, and multi-line code blocks
   come back intact for free.
2. **Plain reflow (fallback).** No confident transcript match (content isn't from a
   recent session, or was copied from somewhere else)? Cleans up the plain text:
   strips the 2-space indent, joins soft-wrapped lines, and keeps paragraph and list
   breaks.
...
Pure stdlib — no dependencies. Clipboard access goes through `pbpaste` (isolated in
`read_clipboard_text`) deliberately, so it runs under any `python3` Alfred resolves
without needing a library installed. Run the built-in tests with `--test`.

### Triggering it from Alfred
`claude-copy.alfredworkflow` is a ready-to-import bundle (generated by
